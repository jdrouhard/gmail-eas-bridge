...
// patched: 'localhost' can resolve to ::1 first on some systems, but
// Dovecot's imap-login/submission-login listeners (conf.d/10-master.conf)
// are pinned to 127.0.0.1 only - a ::1 attempt gets "Connection refused."
// Use the literal IPv4 address to remove the ambiguity entirely.
...
define('IMAP_FOLDER_CONFIGURED', true); // patched: folders below are configured for this stack
...
define('IMAP_DEFAULTFROM', getenv('GMAIL_USER') ?: ''); // patched: pull from env instead of hardcoding
...
define('IMAP_SMTP_METHOD', 'smtp'); // patched: submit via the local Dovecot submission relay

global $imap_smtp_params;
// SMTP Parameters
//      mail : no params
$imap_smtp_params = array('host' => '127.0.0.1', 'port' => 587, 'auth' => true, 'username' => 'imap_username', 'password' => 'imap_password', 'verify_peer' => false, 'verify_peer_name' => false, 'allow_self_signed' => true); // patched: local Dovecot submission on 127.0.0.1:587 (literal IPv4 - see IMAP_SERVER comment above re: localhost/::1), see README.md
//      sendmail
...
// patched: our Dovecot on 127.0.0.1:143 has no TLS (ssl = no - it's
// loopback-only, see config/dovecot/dovecot.conf), so it advertises
// SASL AUTH=PLAIN over what the c-client library considers an
// insecure connection. c-client hardcodes a refusal to use AUTH=PLAIN
// in that situation ("SECURITY PROBLEM: insecure server advertised
// AUTH=PLAIN"), regardless of Dovecot's own disable_plaintext_auth
// setting. Disabling the PLAIN *authenticator* here just makes
// imap_open() fall back to the plain IMAP LOGIN command instead of
// SASL AUTH=PLAIN - same credentials over the wire, same lack of
// encryption (fine, since it's loopback-only), just a code path
// c-client doesn't have this particular refusal baked into.
define('IMAP_DISABLE_AUTHENTICATOR', 'PLAIN');
